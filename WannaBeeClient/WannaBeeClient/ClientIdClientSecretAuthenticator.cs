...
/// <summary>
/// a <see cref="IWannaBeeAuthenticator"/> that is based on a client secret and a client id
/// </summary>
public class ClientIdClientSecretAuthenticator : IWannaBeeAuthenticator
{
    protected const string Auth0Domain = "https://hochfrequenz.eu.auth0.com";
    protected const string WannaBeeScope = "client:default";
    protected const string WannBeeAudience = "https://wannastage.bee";
    ...
    /// <summary>
    /// <inheritdoc cref="IWannaBeeAuthenticator.UseAuthentication"/>
    /// </summary>
    ...
    /// <summary>A <see cref="IWannaBeeAuthenticator"/> that is based on a client secret and a client id</summary>
    ...
    /// <summary>
    /// fetch a token to talk to the wanna bee
    /// </summary>
    ...
    public async Task<string> Authenticate(HttpClient client)
    {
        using (_tokenRequestSemaphore)
        {
            ...
            {
                throw new AuthenticationException(
                    "You must provide a client id and a client secret to the constructor to authenticate with the wanna bee"
                );
            }
            ...
            tokenRequest.Parameters.AddOptional("audience", WannBeeAudience);
